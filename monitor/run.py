    ...
    def run(self, ):
        with picamera.PiCamera() as camera:
            ...
            while True:
                ...
                shape, bb = self.detect_face(image, detector, predictor)
                # print(shape, bb)
                counter += 1
...
'''class Monitor:
    ...
    def report_view_start(self, absolute_time):
        logging.info('Reporting view start.')
        client = requests.session()
        ...
        response = client.post(SERVER_URL + 'monitor/report_view_start/', data=data)
    ...
    def viewer_detection_start(self):
        self.detection_start = self.omxp.position()
        logger.info('Viewer(s) detection start - absolute: {} - relative: {}'.format(time(), self.omxp.position()))

        success = False
        while not success:
            try:
                success = self.report_attention_start(time(), self.omxp.position())
                if not success:
                    logger.info('Problem reporting attention start.')
    ...
    def report_attention_start(self, absolute_time, relative_time):
        logging.info('Reporting attention start.')
        client = requests.session()
        ...
        response = client.post(SERVER_URL + 'monitor/report_attention_start/', data=data)
